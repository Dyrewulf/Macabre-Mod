...
public class ChunkProviderMacabre implements IChunkProvider {
	...
	public ChunkProviderMacabre(World world, long seed, boolean mapFeaturesEnabled)
	{
		this.worldObj = world;
						...
						for (int i3 = 0; i3 < 4; ++i3) {
							...
							for (int k3 = 0; k3 < 4; ++k3) {
								if ((d15 += d16) > 0.0D) {
									...
									blocks[j3 += short1] = Macabre.saltWaterBlock;//these can be set to custom blocks
								} else {
	...
	public void populate(IChunkProvider par1IChunkProvider, int par2, int par3) {
		...
		if (TerrainGen.populate(par1IChunkProvider, worldObj, rand, par2, par3, flag, PopulateChunkEvent.Populate.EventType.ANIMALS)) {
		...
		doGen = TerrainGen.populate(par1IChunkProvider, worldObj, rand, par2, par3, flag, PopulateChunkEvent.Populate.EventType.ICE);
